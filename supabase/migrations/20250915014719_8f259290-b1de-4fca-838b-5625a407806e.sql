...
-- 8. Add security audit trigger for sensitive operations (Fixed: No SELECT triggers)
...
-- Add triggers for sensitive tables (Fixed: Removed SELECT triggers)
...
CREATE TRIGGER audit_payment_methods
  AFTER INSERT OR UPDATE OR DELETE ON public.saved_payment_methods
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_access();